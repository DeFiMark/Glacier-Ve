...
contract RedemptionReceiver is ILayerZeroReceiver {
    ...
    constructor(
        ...
        require(
            _fantomChainId == 12 || _fantomChainId == 10012,
            "CHAIN_ID_NOT_FTM"
        );
    ...
    event Initialized(
        address fantomSender,
        uint256 redeemableUSDC,
        uint256 redeemableVELO
    );
    ...
    function lzReceive(
        ...
        require(VELO.claim(redemptionAddress, shareOfVELO), "CLAIM_FAILED");
    }
    ...
    function claimLeftovers() external onlyTeam {
        require(
            block.timestamp >= deployed + 30 days,
            "LEFTOVERS_NOT_CLAIMABLE"
        );
        require(
